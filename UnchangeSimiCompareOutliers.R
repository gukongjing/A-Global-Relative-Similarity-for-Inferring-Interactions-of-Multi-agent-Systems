...
final = parLapply(cl, p,function(p){
  ...
  for (i in 1:rept) {
    ...
    traj = function(k,td,Nt,N,uav,A,x0,v0){
      ...
      plot(xg[1,25:N,1],xg[1,25:N,2],xlim=c(-1,25),ylim=c(-1,25),xlab = "x-coordinate",ylab = "y-coordinate",type = 'l',
      ...
      temp1[sample(length(temp1),outnum)] = temp1[sample(length(temp1),outnum)]
      dim(temp1) = c(Nt,N,4)
      xg1 = temp1[,,1:2]
      vg1 = temp1[,,3:4]
      ...
      plot(xg1[1,1:N,1],xg1[1,1:N,2],xlim=c(0,20),ylim=c(0,20),xlab = "x-coordinate",ylab = "y-coordinate",type = 'l',
           cex.lab=2,cex.axis=2)   # cex.main=1.5,
      for (z in 2:Nt) {
        lines(xg1[z,1:N,1],xg1[z,1:N,2])
      }
      ...
      plot(xg1[1,25:N,1],xg1[1,25:N,2],xlim=c(-3,3),ylim=c(-3,3),xlab = "x-coordinate",ylab = "y-coordinate",type = 'l',
           cex.lab=2,cex.axis=2)   # cex.main=1.5,
      for (z in 2:Nt) {
        lines(xg1[z,25:N,1],xg1[z,25:N,2])
      }
...
# write.table(D,file = 'Doutliers0324-2.txt', sep = ' ', row.names = FALSE, col.names = FALSE)
D = as.matrix(read.table('Doutliers0324-2.txt',sep = ' ', header = FALSE))
...
library('extrafont')
library(ggplot2)
p2 = ggplot(data = ggdatadens, mapping = aes(x = Density, y = AUC,color = Method,linetype = Type))+
...
library(cowplot)
gg1 <- ggdraw() +
  draw_plot(p1, 0,0,0.5, 1) + draw_plot(p2, 0.5,0,0.5,1)
print(gg1)

# library(cowplot)
# gg1 <- ggdraw() + 
#   draw_plot(p1, 0,0,0.445, 0.93) + draw_plot(p2, 0.445,0,0.555,0.93)+
#   draw_plot_label("A. Invariant topology model", -0.1,1, size = 15,colour = "black")
# print(gg1)
