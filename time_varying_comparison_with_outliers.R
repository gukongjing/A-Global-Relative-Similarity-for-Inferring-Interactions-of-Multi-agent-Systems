...
final = parLapply(cl, rept,function(rept){
  ...
  traj = function(k,td,Nt,N,uav,communication,x0,v0){
    ...
    temp1[sample(length(temp1),outnum)] = temp1[sample(length(temp1),outnum)]/2
    dim(temp1) = c(Nt,N,4)
    xg1 = temp1[,,1:2]
    vg1 = temp1[,,3:4]
...
# write.table(D,file = 'time_varying_comparison_with_outliers.txt', sep = ' ', row.names = FALSE, col.names = FALSE)
D = as.matrix(read.table('time_varying_comparison_with_outliers.txt',sep = ' ', header = FALSE))
...
library('extrafont')
library(ggplot2)
p5 = ggplot(data = ggdatadens, mapping = aes(x = Time, y = AUC,color = Method,linetype = Type))+
...
library(cowplot)
gg3 <- ggdraw() + 
  draw_plot(p4, 0,0,0.5, 1) + draw_plot(p5, 0.5,0,0.5,1)
print(gg3)


######################总绘图##################
# UnchangeSimiCompareWholeForm.R
# UnchangeSimiCompareOutliers.R
# switch_comparison.R
# time_varying_comparison.R
# time_varying_comparison_with_outliers.R

library(cowplot)
gg <- ggdraw() + 
  draw_plot(gg1, 0,0.52,0.65, 0.48) + draw_plot(gg2, 0.68,0.52,0.32,0.48)+ draw_plot(gg3, 0,0,1,0.5)
print(gg)
